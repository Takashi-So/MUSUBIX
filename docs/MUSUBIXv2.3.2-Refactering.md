...
│   │                  分析・検索エンジン                   │ │
│   ├─────────────────────────────────────────────────────┤ │
│   │ • 依存関係分析    • 呼び出し追跡    • 影響範囲分析   │ │
│   │ • GraphRAG検索    • セキュリティ    • リファクタ提案 │ │
│   └─────────────────────────────────────────────────────┘ │
└─────────────────────────────────────────────────────────────┘
```
...
```bash
npm install -g @nahisaho/musubix-codegraph@2.3.2
```
...
cd /tmp/moodle
npm init -y
npm install @nahisaho/musubix-codegraph@2.3.2
```
...
# MCP ツール: find_dependencies
# 指定したクラスが依存しているすべてのモジュールを表示
```
...
```javascript
// MCP ツール: find_callers
// authenticate() メソッドを呼び出している箇所を検索
{
  "tool": "find_callers",
...
```
ユーザー: このプロジェクトの主要なコンポーネントを説明して

AI: [explain_codebase プロンプト使用]
    [global_search ツール使用]
...
```javascript
// MCP ツール: global_search
{
  "tool": "global_search",
  "query": "ユーザー認証のフロー"
}

// コミュニティ（モジュール群）を横断して関連コードを検索
// 単なるキーワード検索ではなく、意味的な関連性で検索

// MCP ツール: local_search
{
  "tool": "local_search",
  "query": "パスワードハッシュ",

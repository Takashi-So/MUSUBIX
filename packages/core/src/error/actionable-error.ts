...
export class ActionableError extends Error {
  ...
  /**
   * Create an error with a single suggestion
   */
  ...
  /**
   * Check if an error is an ActionableError
   */
 ...
 * Error Formatter
 *
 * Formats ActionableError for CLI display with colors and structure.
 */
export class ErrorFormatter {
  ...
  /**
   * Format an ActionableError for display
   */
  ...
  /**
   * Apply color based on severity (ANSI codes)
   */
      ...
      suggestions: [
        {
          ...
          docLink: 'docs/USER-GUIDE.md#ears-format',
        },
        {
          action: 'Run validation',
          description: 'Use the validation command to see detailed issues',
          command: `musubix requirements validate ${file}`,
        },
      ],
    });
  },
    ...
    return new ActionableError(`Cannot transition from ${from} to ${to}: ${reason}`, {
      ...
      suggestions: [
        ...
        },
        {
          ...
          command: 'musubix workflow get-status',
        },
      ],
    });
  },
    ...
    return new ActionableError(`Quality gate failed: ${gate}`, {
      ...
      suggestions: [
        ...
        },
        {
          ...
          command: 'musubix workflow validate-transition',
        },
      ],
    });
  },

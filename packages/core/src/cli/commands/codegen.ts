...
function generateC4ComponentCode(
  ...
  const { id, name, type, description } = component;
  ...
  if (language === 'typescript') {
    // Determine base interface based on component type
    const interfaceName = `I${name}`;
    const methodStubs = generateMethodStubsForComponent(type, description);
 ...
 * Factory function
 */
export function create${name}(): ${interfaceName} {
  return new ${name}();
}
`;
  }
...
/**
 * Generate method stubs based on component type
 */
function generateMethodStubsForComponent(_type: string, description: string): Array<{
  name: string;
  params: string;
  returnType: string;
  description: string;
  ...
  // Common patterns based on description keywords
  ...
  if (lowerDesc.includes('priorit') || lowerDesc.includes('manag')) {
    methods.push(
      {
        name: 'getAll',
        params: '',
